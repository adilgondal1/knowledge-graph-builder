...
class Neo4jManager {
      ...
      // console.log("storing entities")
        ...
        await session.writeTransaction(async tx => {
            for (const person of parsedData.people || []) {
              await this.createOrUpdatePerson(person, emailId);
            }
            
            for (const place of parsedData.places || []) {
              await this.createOrUpdatePlace(place, emailId);
            }
            
            for (const event of parsedData.events || []) {
              await this.createOrUpdateEvent(event, emailId);
            }
   ...
   */
  async createOrUpdatePerson(person, emailId) {
    const session = this.driver.session();
    try {
      const result = await session.writeTransaction(async tx => {
        ...
        ON CREATE SET p.role = $role, 
                     ...
                     p.mentionedInEmails = [$emailId]
        ON MATCH SET p.role = COALESCE(p.role, $role),
                    p.organization = COALESCE(p.organization, $organization),
                    p.mentionedInEmails = CASE
                      WHEN p.mentionedInEmails IS NULL THEN [$emailId]
                      WHEN NOT $emailId IN p.mentionedInEmails THEN p.mentionedInEmails + $emailId
                      ELSE p.mentionedInEmails
                    END
        RETURN p
      `;
   ...
   * @returns {Promise<Object>} The created/updated node data
   */
  async createOrUpdatePlace(place, emailId) {
    const session = this.driver.session();
    try {
      const result = await session.writeTransaction(async tx => {
        const query = `
   ...
   * @returns {Promise<Object>} The created/updated node data
   */
  async createOrUpdateEvent(event, emailId) {
    ...
    try {
      ...
      const query = `
        ...
        ON MATCH SET e.date = COALESCE(e.date, $date),
                    ...
                    e.mentionedInEmails = CASE
                    WHEN e.mentionedInEmails IS NULL THEN [$emailId]
                    WHEN NOT $emailId IN e.mentionedInEmails THEN e.mentionedInEmails + $emailId
                    ELSE e.mentionedInEmails
                  END
        RETURN e
      `;
